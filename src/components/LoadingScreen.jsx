...
export default function LoadingScreen({ onEnter, onComplete }) {
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-[#02101a] overflow-hidden pointer-events-auto transition-all duration-700 ${
      ...
      <div className="relative max-w-[92vw] sm:max-w-2xl w-full p-5 sm:p-12 glass-panel-glow border-2 border-[#00f0ff] rounded-3xl backdrop-blur-2xl bg-[rgba(2,16,26,0.9)] shadow-[0_0_100px_rgba(0,240,255,0.6)] text-center space-y-4 sm:space-y-6 animate-float z-10">
        ...
        <div className="relative inline-block mx-auto max-w-full px-2">
          <img
            ...
            className={`max-w-full h-auto max-h-24 sm:max-h-44 md:max-h-56 object-contain mx-auto drop-shadow-[0_0_45px_rgba(0,240,255,1)] filter transition-all duration-500 ${
              isDiving ? 'scale-125 opacity-0' : 'hover:scale-105'
            }`}
          />
          <div className="absolute -inset-6 bg-[radial-gradient(circle,rgba(0,240,255,0.35)_0%,transparent_75%)] blur-2xl pointer-events-none -z-10 animate-pulse" />
        </div>

        {/* Subtitle Badge */}
        <div className="flex items-center justify-center gap-1.5 sm:gap-3 text-[10px] sm:text-base font-mono text-[#00f0ff] tracking-[0.2em] sm:tracking-[0.4em] font-bold">
          <Sparkles className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#00f0ff] animate-spin shrink-0" />
          <span className="truncate">LEGENDS BENEATH THE SURFACE</span>
          <Sparkles className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#00f0ff] animate-spin shrink-0" />
        </div>

        {/* Date & Venue Tagline */}
        <p className="font-body text-[11px] sm:text-sm text-[#e8f9ff] leading-relaxed max-w-lg mx-auto font-medium">
          National Institute of Technology • October 24-27, 2026
        </p>

        {/* Sonar Loading Progress Bar / Enter Button */}
        {!isLoaded ? (
          <div className="space-y-2.5 max-w-md mx-auto pt-1 sm:pt-2">
            <div className="flex items-center justify-between text-[11px] sm:text-xs font-mono text-[#00d9d0] font-bold">
              <span className="flex items-center gap-1.5 truncate">
                <Radio className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#00f0ff] animate-pulse shrink-0" />
                <span className="truncate">AWAKENING 3D OCEAN WORLD...</span>
              </span>
              <span className="text-[#00f0ff] ml-2">{progress}%</span>
            </div>
          ...
          <div className="pt-1 sm:pt-2 animate-fadeIn">
            <button
              onClick={handleEnterClick}
              className="btn-rune text-xs sm:text-base py-3 sm:py-4 px-5 sm:px-10 w-full sm:w-auto justify-center rounded-full shadow-[0_0_50px_rgba(0,240,255,0.8)] animate-pulse cursor-pointer relative z-20 pointer-events-auto group"
            >
              <Volume2 className="w-4 h-4 sm:w-5 sm:h-5 text-[#00f0ff] group-hover:animate-bounce shrink-0" />
              <span>ENTER THE 3D OCEAN KINGDOM</span>
              <ArrowRight className="w-4 h-4 sm:w-5 sm:h-5 text-[#00f0ff] group-hover:translate-x-1 transition-transform shrink-0" />
            </button>
          </div>
        )}
